...
class LipForensicsModel(DeepfakeModel):
    def load(self):
        try:
            from lipforensics import LipForensics
            self.model = LipForensics().to(self.device)
            self.model.eval()
            logger.info("LipForensics loaded on {}", self.device)
        except Exception as e:
            logger.exception("Failed to load LipForensics: {}", e)
            self.model = None
    def warmup(self):
